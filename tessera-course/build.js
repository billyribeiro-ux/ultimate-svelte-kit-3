 ...
 * prev/next links.
 *
 * The fourth course to use this builder unchanged, which is the point: the
 ...
 *
 * Run:  node tessera-course/build.js
 */
...
const COURSE = {
	name: 'Building Tessera',
	subtitle: 'a SvelteKit 3 course',
	description:
		'Build a real-time collaborative canvas with SvelteKit 3 — conflict-free data types written from scratch, local-first storage, live presence, and an editor that keeps working with the network unplugged.',
	lede: `A collaborative canvas. Two people draw on the same board at the same
		moment and both edits survive. One of them goes through a tunnel, keeps
		working, and everything they did arrives intact when the signal comes back.
		No server decides who wins, because no server is asked. Built from an empty
		folder, with every bug found along the way written down.`,
	badges: ['SvelteKit 3', 'Svelte 5 runes', 'CRDTs from scratch', 'Local-first'],
	closing: `Every chapter shows the real code from the finished project in
		<code>tessera</code>. Type it out rather than pasting where you can — the
		errors you make and fix are the part that sticks. A dozen chapters end with a
		bug that was genuinely in this codebase, what it looked like from the
		outside, and how it was cornered.`
};
