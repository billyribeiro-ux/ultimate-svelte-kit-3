 ...
 * prev/next links.
 *
 * The seventh course to use this builder. Code blocks carry the line range they
 ...
 *
 * Run:  node abacus-course/build.js
 */
...
const COURSE = {
	name: 'Building Abacus',
	subtitle: 'a Svelte 5 and SvelteKit 3 course',
	description:
		'Build a spreadsheet in the browser with Svelte 5 and SvelteKit 3 — a formula language, an incremental dependency-graph engine, a virtualised grid, passkeys instead of passwords, live collaboration over a live query, streaming CSV in a worker, and a container that deploys it.',
	lede: `A formula language with a parser and fifty functions, an engine that
		recalculates only what changed, a grid that scrolls a million rows, passkeys,
		a second person editing beside you, and the Dockerfile that ships it — built
		from an empty folder, using every part of Svelte 5 and SvelteKit 3 that earns
		its place, including everything the September 2026 releases added. A
		spreadsheet is where reactivity was invented, and every chapter says so.`,
	badges: ['Svelte 5.57', 'SvelteKit 3', 'Passkeys', 'adapter-node'],
	closing: `Every code block in this course is read out of the finished project
		in <code>abacus</code> at build time — file and line numbers included — so
		...
		Where the project made a decision, the chapter says what the alternative
		was and why it lost.`
};
