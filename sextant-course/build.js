 ...
 * prev/next links.
 *
 * The fifth course to use this builder, and the first to change it: code blocks
 * now carry the line range they were read from, because they are read out of the
 * project at build time rather than pasted in. See `content/quote.js`.
 *
 ...
 *
 * Run:  node sextant-course/build.js
 */
...
const COURSE = {
	name: 'Building Sextant',
	subtitle: 'a SvelteKit 3 course',
	description:
		'Build a self-hosted observability platform with SvelteKit 3 — a real query language with a lexer, a Pratt parser and a type checker, mergeable sketches, trace assembly, and an interface dense enough to work at three in the morning.',
	lede: `Logs, traces and metrics, and a query language to ask them questions.
		Not a wrapper around somebody else's engine: a hand-written lexer, a Pratt
		parser, a type checker that knows a duration is not a number, an evaluator,
		and a planner that pushes what it can into SQL and is honest about the rest.
		Built from an empty folder, with every bug found along the way written down.`,
	badges: ['SvelteKit 3', 'Svelte 5 runes', 'A real query language', 'Canvas & workers'],
	closing: `Every code block in this course is read out of the finished project
		in <code>sextant</code> at build time — file and line numbers included — so
		...
		A dozen chapters end with a bug that was genuinely in this codebase, what it
		looked like from the outside, and how it was cornered.`
};
