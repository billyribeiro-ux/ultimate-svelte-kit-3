...
/*
 * The key primitives live in `keys.ts`, which imports nothing.
 *
 * They are re-exported here so that every caller still says `access.ts` and
 * nothing has to know about the split — the split exists so that the seed script
 * and the tests can use them under plain Node, where importing this file (and
 * therefore the database, and therefore `$app/env`) is not possible.
 */
export { hashKey, newKey, prefixOf } from './keys.ts';
