 ...
 *
 * **Unsupported languages.** `[[lang=locale]]` is matched by `params/locale.ts`,
 * so `/de/boards` matches nothing and 404s. A German speaker whose browser sent
 * them there deserves the application in English rather than an error page, so
...
export const reroute: Reroute = ({ url }) => {
	...
	if (language && !(LOCALES as readonly string[]).includes(language[1]!)) {
