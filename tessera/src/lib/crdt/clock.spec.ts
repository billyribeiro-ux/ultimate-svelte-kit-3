...
describe('observing', () => {
	...
	it('refuses a timestamp from a machine whose clock is wildly ahead', () => {
		/*
		 * Without this guard one bad message drags the document's clock into the
		 * future permanently: no correct local time can pull an HLC back.
		 */
		const harness = fakeClock('a', 1_000_000);
		const fromTheFuture = stamp(1_000_000 + MAX_DRIFT_MS + 1, 0, 'b');

		expect(() => harness.clock.observe(fromTheFuture)).toThrow(ClockDriftError);
		expect(harness.clock.peek().wall).toBe(0); // untouched
	});

	it('accepts skew inside the limit', () => {
		const harness = fakeClock('a', 1_000_000);
		const skewed = stamp(1_000_000 + MAX_DRIFT_MS - 1, 0, 'b');

		harness.clock.observe(skewed);
		expect(harness.clock.peek().wall).toBe(1_000_000 + MAX_DRIFT_MS - 1);
	});
